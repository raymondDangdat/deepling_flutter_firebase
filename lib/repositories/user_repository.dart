...
class UserRepository {
  ...
  listenToCurrentUser(String uid) {
    final snapshot = FirebaseFirestore.instance.collection('deeplinkUsers').doc(uid).snapshots();
  ...
  Future<User?> registerUser(String name, String email, String password, {String referrerCode = ''}) async {
    ...
    await FirebaseFirestore.instance.collection('deeplinkUsers').doc(uid).set({
      'name': name,
      'email': email,
      'uid': uid,
  ...
  Future<User> getUser(String uid) async {
    DocumentSnapshot userSnapshot = await FirebaseFirestore.instance.collection('deeplinkUsers').doc(uid).get();
  ...
  Future<User> getReferrerUser(String referCode) async {
    final docSnapshots =
        await FirebaseFirestore.instance.collection('deeplinkUsers').where('refer_code', isEqualTo: referCode).get();
  ...
  Future<void> rewardUser(String currentUserUID, String referrerCode) async {
    try {
      ...
      if (!checkIfUserAlreadyExist.exists) {
        ...
        await FirebaseFirestore.instance.collection('deeplinkUsers').doc(referer.uid).update({
